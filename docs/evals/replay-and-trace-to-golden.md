...
Both features are API-key-free by default. They do not call models, import
provider SDKs, execute handlers, store raw prompts, or keep raw provider
payloads.
...
Do not put raw prompts, raw provider payloads, raw tool outputs, secrets,
customer records, tenant IDs, account IDs, or stack traces in replay fixtures.
...
`policy_only` reruns AICF policy decisions against the current manifests and
redacted decision context.
...
`router_only` reruns deterministic capability selection and checks whether the
selected capability slice changed.

`tool_validation_only` validates recorded tool-call arguments against the
current capability input schemas.

`provider_live` is guarded. It refuses by default unless the host explicitly
enables live replay and provides its own runner/client.
...
review-required in `extensions.aicf_replay`. Review the draft before moving it
under `examples/**/evals/`.
...
- response text must not expose private diagnostics, raw prompts, provider
...
Host applications own production trace collection, storage, retention, review
workflow, and any live-provider replay runner. AICF only provides public-safe
contracts and deterministic comparison helpers.
